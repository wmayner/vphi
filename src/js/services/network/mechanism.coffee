###
# services/graph/mechanism.coffee
###

utils = require '../../utils'

sum = utils.sum
bit = utils.bit
negate = utils.negate

andGate = (input) ->
  return bit(sum(input) is input.length)

orGate = (input) ->
  return bit(sum(input) >= 1)

parityGate = (input) ->
  return negate(sum(input) % 2)


module.exports =
  'AND': andGate

  'NAND': (input) ->
    return negate(andGate(input))

  'OR': orGate

  'NOR': (input) ->
    return negate(orGate(input))

  'XOR': (input) ->
    # TODO document that XOR here means the parity interpretation
    return negate(parityGate(input))

  'COPY': (input) ->
    # if input.length > 1
    #   throw "COPY gate may not have more than one input"
    return orGate(input)

  'NOT': (input) ->
    # if input.length > 1
    #   throw "NOT gate may not have more than one input"
    return negate(orGate(input))

  'RAND': (input) ->
    return bit(Math.random() < 0.5)

  'MAJ': (input) ->
    return bit(sum(input) > (input.length / 2))

  'MIN': (input) ->
    # TODO should this be strictly less than half, or just not a majority?
    return bit(sum(input) <= (input.length / 2))

  'PAR': parityGate

  '>': (input, threshold) ->
    return bit(sum(input) > threshold)

  '<': (input, threshold) ->
    return bit(sum(input) < threshold)

keys = (key for own key, val of module.exports)
module.exports.keys = keys
module.exports.names =
  'AND': 'AND Gate'
  'NAND': 'NAND Gate'
  'OR': 'OR Gate'
  'NOR': 'NOR Gate'
  'XOR': 'XOR Gate'
  'COPY': 'COPY Gate'
  'NOT': 'NOT Gate'
  'RAND': 'Random'
  'MAJ': 'Majority'
  'MIN': 'Minority'
  'PAR': 'Parity'
  '>': 'Greater than threshold'
  '<': 'Less than threshold'

# Mechanisms which expect a single input
module.exports.singleInput = ['COPY', 'NOT']
